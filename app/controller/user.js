...
class UserController extends Controller {
    ...
    async patch_updateUser() {
        ...
        try {
            ...
            if (data.length > 0) {
                ...
                const updateStatus = await service.user.updateUserInfo(
                    updateParams
                )
    ...
    async post_upload_avatar() {
        ...
        if (!typeReg.test(getFile.filename))
            return (ctx.body = no_data_failed(
                100,
                '上传的文件只能是jpg png jpeg 后缀的'
            ))
        const arr = getFile.filename.split('.')
        const suffix = arr[arr.length - 1]
        const setFileName = `${userId}.${suffix}`
        // 文件路径
        const taskFileUrl = path.join(
            __dirname,
        ...
        const getAllAvatar = fs.readdirSync(
            path.join(__dirname, `../public/upload/avatar/`)
        )
        // 2. 找到当前的用户之前上传过的头像
        const getUserIdAvatar = getAllAvatar.find(
            (item) => item.split('.')[0] == userId
        )
        // 3. 如果存在图片就删除
        if (getUserIdAvatar)
            fs.unlinkSync(
